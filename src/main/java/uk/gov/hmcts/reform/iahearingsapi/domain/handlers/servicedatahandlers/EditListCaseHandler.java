...
public class EditListCaseHandler extends ListedHearingService implements ServiceDataHandler<ServiceData> {
    ...
    private boolean tryUpdateListCaseHearingDetails(AsylumCase asylumCase, ServiceData serviceData, String caseId) {
        ...
        boolean isNotRemoteToRemoteChannelUpdate = !(currentChannelIsRemote && nextChannelIsRemote);
        //Location is updated if hearing channel is updated except for remote to remote channel update
        boolean updateLocationBasedOnChannelUpdate = hearingChannelUpdated && isNotRemoteToRemoteChannelUpdate;
        boolean hearingLocationUpdated = tryUpdateHearingCentre(
            asylumCase, serviceData, updateLocationBasedOnChannelUpdate, hearingId);
        ...
        boolean triggerEventForHearingChannelUpdate = hearingChannelUpdated && isNotRemoteToRemoteChannelUpdate;
        if (triggerEventForHearingChannelUpdate || hearingLocationUpdated || hearingDateTimeUpdated) {
