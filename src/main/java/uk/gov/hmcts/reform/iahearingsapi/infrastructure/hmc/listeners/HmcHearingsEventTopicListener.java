...
import uk.gov.hmcts.reform.iahearingsapi.infrastructure.hmc.ProcessHmcMessage;
...
public class HmcHearingsEventTopicListener {
    ...
    private final ProcessHmcMessage processHmcMessage;

    public HmcHearingsEventTopicListener(@Value("${ia.hmctsServiceId}") String hmctsServiceId,
                                         ProcessHmcMessage processHmcMessage) {
        this.hmctsServiceId = hmctsServiceId;
        this.processHmcMessage = processHmcMessage;
        this.objectMapper = new ObjectMapper();
        objectMapper.registerModule(new JavaTimeModule());
    }

    @JmsListener(
        destination = "${azure.service-bus.hmc-response-subscription.topicName}",
        subscription = "${azure.service-bus.hmc-response-subscription.subscriptionName}",
    ...
    public void onMessage(JmsBytesMessage message) throws JMSException,
        ...
        try {
            ...
            if (isMessageRelevantForService(hmcMessage)) {
                ...
                processHmcMessage.processEventMessage(hmcMessage);
            }
