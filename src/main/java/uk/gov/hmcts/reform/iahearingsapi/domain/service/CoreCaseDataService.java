...
import uk.gov.hmcts.reform.iahearingsapi.domain.service.dto.SubmitEventParameters;
...
public class CoreCaseDataService {
    ...
    private static final String UNAUTHORIZED_ACCESS_MESSAGE = "Unauthorized access to getCaseById: {}";
    private static final String CASE_DETAILS_FOUND_MESSAGE = "Case details found for the caseId: {}";
    ...
    public AsylumCase getCaseFromStartedEvent(StartEventResponse startEventResponse) {
        ...
        return caseDetails != null ? iaCcdConvertService.convertToAsylumCaseData(caseDetails.getData()) : null;
    }
    ...
    public CaseDetails triggerSubmitEvent(Event event,
        ...
        log.info(CASE_DETAILS_FOUND_MESSAGE, caseId);
        SubmitEventParameters parameters = new SubmitEventParameters(getUserToken(event, caseId),
                                               getS2sToken(event, caseId),
                                               getUid(event, caseId),
                                               caseId,
                                               asylumCase,
    ...
    private CaseDetails submitEventForCaseWorker(SubmitEventParameters parameters) {
        StartEventResponse startEventResponse = startCaseEvent(parameters.event(),
                                                               parameters.caseId(), parameters.caseType());
        ...
        if (startEventResponse.getCaseDetails().getLastModified().truncatedTo(ChronoUnit.MILLIS)
            .isAfter(parameters.lastModified())) {
            throw new ConcurrentModificationException(
                String.format("Case with ID %s cannot be updated: case details out of date", parameters.caseId()));
        }

        CaseDataContent request = buildRequestContent(parameters.caseId(),
                                                      parameters.data(),
                                                      parameters.event(),
                                                      parameters.ignoreWarning(),
    ...
    private static CaseDataContent buildRequestContent(String caseId,
    ...
    private String getUserToken(Event event, String caseId) {
        ...
        try {
            ...
            log.error(UNAUTHORIZED_ACCESS_MESSAGE, ex.getMessage());
