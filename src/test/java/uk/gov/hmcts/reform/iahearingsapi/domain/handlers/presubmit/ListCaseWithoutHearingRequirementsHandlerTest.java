...
public class ListCaseWithoutHearingRequirementsHandlerTest {
    ...
    @Mock
    private HmcHearingRequestPayload hmcHearingRequestPayload;
    ...
    void setUp() {
        ...
        handler = new ListCaseWithoutHearingRequirementsHandler(hearingService, serviceHearingValuesProvider);
    }
    ...
    void should_send_hearing_creation_request_to_hmc() {
        when(serviceHearingValuesProvider.buildAsylumAutoHearingPayload(caseDetails))
            .thenReturn(hmcHearingRequestPayload);

        handler.handle(ABOUT_TO_SUBMIT, callback);

        verify(hearingService, times(1)).createHearing(hmcHearingRequestPayload);
    ...
    void should_throw_exception_if_call_unsuccessful() {
        when(serviceHearingValuesProvider.buildAsylumAutoHearingPayload(caseDetails))
            .thenReturn(hmcHearingRequestPayload);
        when(hearingService.createHearing(hmcHearingRequestPayload)).thenThrow(new IllegalStateException());
